...
<GlossaryTerm term="Maru validator">Maru validator</GlossaryTerm> set that can tolerate the loss of one region
under <GlossaryTerm term="Quorum Byzantine Fault Tolerance (QBFT)">QBFT</GlossaryTerm> (`n=4`, `f=1`).
...
flowchart TB
  ...
  subgraph Regions["Regions"]
    direction LR
    V1["Region 1 (primary)<br/>Sequencer node + RPC +<br/>finalization pipeline (active)"]:::core
    V2["Region 2 (standby)<br/>Sequencer node + RPC +<br/>finalization pipeline (warm standby)"]:::core
    V3["Region 3<br/>Sequencer node + RPC"]:::core
    V4["Region 4<br/>Sequencer node + RPC"]:::core
  end
...
Each region runs a <GlossaryTerm term="Sequencer node">sequencer node</GlossaryTerm>: a
<GlossaryTerm term="Maru validator" /> paired with its own
<GlossaryTerm term="Linea Besu" /> execution client.
...
required.
For QBFT topology and latency, see [Multi-validator consensus](./multi-validator-consensus.mdx).
...
- Learn how [multi-validator consensus](./multi-validator-consensus.mdx) improves
  <GlossaryTerm term="Sequencer">sequencer</GlossaryTerm> fault tolerance and learn how to
  [run multiple Maru validators](../how-to/run-multiple-validators.mdx).
