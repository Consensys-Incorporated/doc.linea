...
Access control limits who can use the operator's RPC, but it does not hide chain data from the
operator who runs the node or stores the data.
It does not make public chain data private, replace zero-knowledge proofs, or provide
data availability.
...
A restricted deployment exposes one RPC endpoint (the access control endpoint).
...
You can configure access control on a public or private deployment.
...
The endpoint restricts who can use the operator's RPC;
It does not hide data that is already onchain.

In a <GlossaryTerm term="Validium">private validium</GlossaryTerm>, transaction data is
kept offchain, so the endpoint is also the path to chain data.
...
- [Trust model](../evaluate/trust-model.mdx): What participants can verify when access and
