...
<GlossaryTerm term="Validium">private validium</GlossaryTerm> restricts JSON-RPC access.
...
It does not restrict access to data that is already onchain, hide data from the operator who runs
the node or stores it, replace zero-knowledge proofs, or provide data availability.
Operators typically configure access control on a private validium, where transaction data stays offchain.

:::

## Access control stack

A restricted <GlossaryTerm term="Validium">private validium</GlossaryTerm> exposes one RPC endpoint
