...
5. In the commit phase, the destination asks its access control endpoint whether that
   ...
   The endpoint evaluates the caller of the cross-chain call against the destination's own
   RBAC policy: the source contract on the source chain (a chain-based decentralized ID such
   as `did:pkh:eip155:<source-chain>:<address>`), not `tx.origin` of the source transaction.
...
sequenceDiagram
  ...
  DestEP->>DestEP: Check grants for msg.sender (chain-based DID)
  alt Allowed
    DestEP-->>Dest: Allow
    Dest-->>Src: Commit
