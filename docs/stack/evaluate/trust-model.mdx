...
- The <GlossaryTerm term="System contracts">system contracts</GlossaryTerm> can pause subsystems, change the verifier address, change rate limits,
  ...
  <GlossaryTerm term="Canonical token bridge">canonical bridge</GlossaryTerm>, pause and upgrade [contract roles](#system-contract-roles)).
...
- **Sequencer halt**: Block production stops, and pending transactions are not included.
  In a [multi-validator](../deployment/multi-validator-consensus.mdx) deployment,
  [restore lost quorum](../how-to/run-multiple-validators.mdx#restore-lost-quorum).
